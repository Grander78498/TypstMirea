...
#figure(image("img/prac1_2024-12-23-23-49-09.png", height: 100%))
#show: template

#outline(target: selector(heading.where(level: 1))
...
#print_symbols(
($X$, "конечное множество концептов (понятий, терминов) предметной
области, которую представляет онтология"),
...
#figure(image("image.png", height: 35%), caption: [Схема онтологии "Музыкальная индустрия"])<ont-schema>
...
#figure(image("image (1).png", height: 25%), caption: [Составленная иерархия классов])<class-hier>
...
#figure(image("album.png"), caption: [Слоты класса "Альбом"])<album-class>

На Рисунке @song-class представлены слоты класса "Песня". Слотами данного класса являются: альбом и название.

#figure(image("2024-12-20-23-33-07.png"), caption: [Слоты класса "Песня"])<song-class>
...
#figure(image("2024-12-20-23-35-01.png"), caption: [Слоты класса "Музыкант"])<musician-class>

На Рисунке @group-class представлены слоты класса "Группа". Слотами данного класса являются страна и название.

#figure(image("2024-12-20-23-35-58.png", height: 10%), caption: [Слоты класса "Группа"])<group-class>
...
#figure(image("2024-12-20-23-36-54.png"), caption: [Экземпляры класса "Группа"])<group-ex>

На Рисунке @musician-ex представлены экземпляры класса "Музыкант" и значения полей в одном из них.
 
#figure(image("2024-12-20-23-52-35.png"), caption: [Экземпляры класса "Музыкант"])<musician-ex>

На Рисунке @song-ex представлены экземпляры класса "Песня" и значения полей в одном из них.
 
#figure(image("2024-12-20-23-55-34.png"), caption: [Экземпляры класса "Песня"])<song-ex>

На Рисунке @song-ex представлены экземпляры класса "Альбом" и значения полей в одном из них.
 
#figure(image("2024-12-20-23-56-45.png"), caption: [Экземпляры класса "Альбом"])<album-ex>
== Выполнение запросов в Protege
Программа Protégé позволяет составлять запросы на получение объектов по определённым условиям, а также вытаскивать связанные объекты для уже полученных объектов. Проделан обычный запрос на получение экземпляров (Рисунок 1.4.1).
...
Для работы с онтологиями написана программа на языке Python, которая запускается в консоли и поддерживает выполнение запросов на получение экземпляров. Её код представлен в Листинге @app На Рисунке 1.5.1 представлен результат выполнения запроса музыкантов в группе, выполненный в программе.
